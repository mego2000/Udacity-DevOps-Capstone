pipeline {
    ...
    stages {
        ...
        stage('Build Green Docker Image') {
            steps {
                script{
                    greenDockerImage = docker.build "mohamedghonim/pre-production-flask-app"
                }
            }
        }
        ...
        stage('Clean Up Green Image'){
            steps { 
                sh "docker rmi mohamedghonim/pre-production-flask-app:latest" 
            }
        }
        ...
        stage('Build Blue Docker Image') {
            steps {
                script{
                    blueDockerImage = docker.build "mohamedghonim/flask-app"
                }
            }
        }
        ...
        stage('Clean Up Blue Image'){
            steps { 
                sh "docker rmi mohamedghonim/flask-app:latest" 
            }
        }
